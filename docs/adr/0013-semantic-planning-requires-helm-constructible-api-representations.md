...
must not invent API availability for that invocation. This does not
guarantee that Kubernetes will accept the CRD, that discovery appears
at runtime, or that a dependent resource succeeds.
