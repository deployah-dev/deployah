...
func TestPrepareChart_CacheSurvivesCallerCleanup(t *testing.T) {
	...
	returnedPath, err := PrepareChart(t.Context(), resolved, cache, nil)
	require.NoError(t, err)
...
func TestPrepareChart_RequiresCache(t *testing.T) {
	t.Parallel()
	_, err := PrepareChart(t.Context(), nil, nil, nil)
	require.Error(t, err)
...
func TestPrepareChart_CanceledContextReturnsImmediately(t *testing.T) {
	...
	cancel()
	_, err := PrepareChart(ctx, nil, NewChartCache(time.Hour), nil)
...
func TestPrepareChart_RequiresResolvedSpec(t *testing.T) {
	t.Parallel()
	_, err := PrepareChart(t.Context(), nil, NewChartCache(time.Hour), nil)
	require.Error(t, err)
